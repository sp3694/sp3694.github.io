...
rest_inspec %>% 
  ...
  layout(title = "Number of restaraunt ratings by neighborhood",
...
rest_inspec %>% 
  ...
  plot_ly(y = ~score, color = ~boro, type = "box",
          colors = "Set2") %>% 
  layout(title = "Restaraunt ratings by neighborhood",
      xaxis = list(title = "neighborhood"), 
...
rest_inspec  %>% 
  ...
  layout(yaxis = list(
      type = 'date',
      range = c("2014","2017"))) %>% 
  layout(title = "Restaraunt ratings by inspection date and neighborhood with inspection violation description",
